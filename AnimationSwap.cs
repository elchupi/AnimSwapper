...
public static unsafe class AnimationSwap
{
    ...
    // ── Penumbra IPC ───────────────────────────────────────────
    private const string PenumbraTag = "AnimSwapper_AnimSwap";
    ...
    private const string JobPenumbraTag = "AnimSwapper_JobAnimSwap";
    ...
    {
        bool raceEnabled = AnimSwapper.Config.EnableAnimationSwaps;
        bool jobEnabled  = AnimSwapper.Config.EnableJobAnimationSwaps;
    ...
    private static void UpdateJobSwapState(Dalamud.Game.ClientState.Objects.Types.IGameObject lp)
    {
        ...
        foreach (var rule in AnimSwapper.Config.JobAnimSwapRules)
        {
    ...
    private static void UpdateSwapState(Dalamud.Game.ClientState.Objects.Types.IGameObject lp)
    {
        ...
        foreach (var rule in AnimSwapper.Config.AnimationSwapRules)
        {
            if (!rule.Enabled) continue;
    ...
    public static void FlushDiag()
    {
        try
        {
            ...
            sb.AppendLine($"── Configured rules ({AnimSwapper.Config.AnimationSwapRules.Count}) ──");
            sb.AppendLine($"   (matching against visual race {matchRace} = {LookupRaceName(matchRace)})");
            for (int i = 0; i < AnimSwapper.Config.AnimationSwapRules.Count; i++)
            {
            ...
            sb.AppendLine($"  Job swap rules ({AnimSwapper.Config.JobAnimSwapRules.Count}):");
            for (int i = 0; i < AnimSwapper.Config.JobAnimSwapRules.Count; i++)
            {
                var jr = AnimSwapper.Config.JobAnimSwapRules[i];
            ...
            try
            {
                ...
                if (diagJobFolder != null)
                {
                    ...
                    foreach (var jr in AnimSwapper.Config.JobAnimSwapRules)
                    {
